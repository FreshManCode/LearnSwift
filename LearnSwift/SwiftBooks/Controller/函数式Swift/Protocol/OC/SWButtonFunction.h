...
/*  iOS编程官方现在提供了两种语言:
 ...
 面向对象的三大特点：继承、封装、多态，其中继承的最大优点就是代码复用。但是有些时候继承如果没有限制很可能会被滥用，造成代码结构散乱，分散到各个类中，如果想要做功能迁移，可能会拔出萝卜带出泥，高耦合也是继承无法避免的问题。另外，后期维护困难，如果新人加入项目，那么掌握各个父类中的功能也是一项不小的成本。
 对于这些大家应该都比较了解,就不多做介绍,下面接入今天的正题, iOS面向协议的编程.
 
 可能对于不了解iOS语言特性的同学就比较困惑了,只知道面向接口编程,难道iOS比较特殊?非常规语言?并不是iOS比较特殊,而是准确的
 来说iOS语言中没有接口这一概念,也就是不像其它面向对象语言,比如:Java有Interface关键词.难道iOS就不能使用如此强大的功能了?
