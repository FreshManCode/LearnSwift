...
//        twoAddSum([2,4,3], [5,6,4])
    ...
    func twoAddSum(_ num1: [Int], _ num2: [Int]) -> [Int] {
        ...
        return transferNumberToReverseArray(result)
    }
    ...
    func transferNumberToReverseArray(_ number:Int) -> [Int] {
    ...
    func transListNodeToListArray(_ node:ListNode?) -> [Int] {
        if node != nil {
...
//        var index = 0
        let array1 = [1,2,3]
        let node = createNode(array1)
        let nodeResult = transListNodeToListArray(node)
        printLog("nodeResult is:\(nodeResult)")
        
    }
    
    func createNode(_ numbers:[Int]) -> ListNode? {
        var node:ListNode?
        ...
        while index < length {
            ...
            if index + 1 < length {
                nextNode = ListNode.init(numbers[index + 1])
            }
            if index == 0 {
                node = my_node
                node?.next = nextNode
            }
            my_node.next = nextNode
            nextNode = my_node
            index += 1
        }
        return node
